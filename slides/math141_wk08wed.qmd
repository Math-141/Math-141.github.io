...
-   Midterm logistics:
    - Review of score distribution
    - Corrections to written exam
...
```{r}
esp_dat <- data.frame(
  guess_correct = sample(c(0,1), size = 329, replace = TRUE, prob = c(0.75, 0.25))
)
head(esp_dat)
```
...
```{r}
esp_dat %>%
  specify(response = guess_correct) %>%
  calculate(stat = "mean")
```
:::
...
```{r}
guessing_samp_dist <- esp_dat %>%
  specify(response = guess_correct) %>%
  generate(reps =  1000, type = "bootstrap") %>%
  calculate(stat = "mean")

head(guessing_samp_dist)
```
::::
...
```{r, eval = FALSE}
reedie_dat <- data.frame(
  guess_correct = sample(c(0,1), size = 329, replace = TRUE, prob = c(0.75, 0.25))
)

p_hat <- 106 / 329

guessing_samp_dist <- reedie_dat %>%
  specify(response = guess_correct) %>%
  generate(reps =  1000, type = "bootstrap") %>%
  calculate(stat = "mean")


ggplot(data = guessing_sampling_dist, mapping = aes(x = stat)) +
...
guessing_samp_dist <- esp_dat %>%
  ...
  generate(reps =  1000, type = "bootstrap") %>%
  calculate(stat = "mean")
ggplot(data = guessing_samp_dist, mapping = aes(x = stat)) +
...
ggplot(data = guessing_samp_dist, mapping = aes(x = stat)) +
  geom_histogram(color = "white", bins = 20) +
  geom_vline(xintercept = 0.32, color = "orange", size = 2)
```
:::
:::::
