...
**Interpretation:** The point estimate is `r round(x_bar$stat, digits = 2)`mm. I am 95% confidence that the average bill length of Adelie penguins is between `r round(ci$lower_ci, digits = 2)`mm and `r round(ci$upper_ci, digits = 2)`mm.
...
**Interpretation**: The point estimate is `r round(diff_x_bar$stat, digits = 2)`mm. I am 95% confidence that the difference in bill length between Adelie and Chinstrap penguins is, on average, between `r round(ci_95$lower_ci, digits = 2)`mm and `r round(ci_95$upper_ci, digits = 2)`mm.
