...
```{r}
names(trees) <- c("species", "dbh", "acceptable.stock")
colnames(trees)
```

```{r}
names(trees) <- c("Species", "DBH", "AGS")
```
...
```{r}
loblolly <- read.csv("Loblolly.csv")

head(loblolly) #tail(loblolly)
```

### Accessing specific elements of data frames.

```{r}
loblolly[1,3] # Row 1 and column 3.
loblolly[1:3, 3] # Rows 1 through 3 and column 3.
...
```{r, error=TRUE}
height # Does not exist outside the data frame.